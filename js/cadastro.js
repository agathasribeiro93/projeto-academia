...
class validacao {
    ...
    equal(input, inputName){
        let inputComparacao = document.getElementsByName(inputName)[0]

        let erroMsg = `As senhas devem ser iguais`

        if(input.value != inputComparacao.value){
            this.printMsg(input, erroMsg)
        }
    }
...
const getViaCepEnd = async (estado, cidade, rua) => {
    ...
    cepInput.value = data[0].cep.replace('-', '')
...
//EVENTO - CHECKBOX NÃO SEI MEU CEP
